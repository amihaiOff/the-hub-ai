...
jest.mock('@/lib/auth-api-key', () => ({
  getTasksHouseholdIdFromToken: jest.fn(),
...
const mockTokenHousehold = getTasksHouseholdIdFromToken as jest.MockedFunction<
  typeof getTasksHouseholdIdFromToken
>;
...
describe('resolveTasksAccess', () => {
  ...
  it('prefers the session: returns the active household + current user', async () => {
    ...
    const result = await resolveTasksAccess(req());
  ...
  it('falls back to the token: household + resolved owner user', async () => {
    ...
    const result = await resolveTasksAccess(req());
    // Attribution: a token-created task is owned by the household owner.
