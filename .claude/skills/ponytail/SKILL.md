...
This skill governs code shape (YAGNI, minimal diffs, reuse before new code). It does not
override CLAUDE.md's mandatory Debug → Code → Test → Review workflow — the lazy solution
still gets the full `testing-agent` + `reviewer-agent` pass before it's done. "One runnable
check" above is ponytail's own floor, not a substitute for this repo's test requirements.

Source: https://github.com/DietrichGebert/ponytail (MIT license), added by request on
2026-09-20.
