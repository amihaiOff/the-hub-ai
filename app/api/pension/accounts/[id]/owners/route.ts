...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const owners = await prisma.pensionAccountOwner.findMany({
      where: { accountId: id },
      include: {
        profile: {
          select: {
...
/**
 * PUT /api/pension/accounts/[id]/owners
 * Update owners of a pension account (replace all owners)
 */
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updatedOwners = await prisma.pensionAccountOwner.findMany({
      where: { accountId: id },
      include: {
        profile: {
          select: {
